...
import { ActionFunc, PredActionList, PredFunc } from '../util/types';
...
const SearchBar = (props: SearchBarProps): JSX.Element | null => {
  ...
  const spacePred: PredFunc = (event: KeyboardEvent): boolean => {
    return !searchDisplayed && (event.keyCode === KEYCODES[SPACEBAR]);
  };
  const escapePred: PredFunc = (event: KeyboardEvent): boolean => {
    return searchDisplayed && (event.keyCode === KEYCODES[ESCAPE]);
  };
  const enterPred: PredFunc = (event: KeyboardEvent): boolean => {
    return searchDisplayed && (event.keyCode === KEYCODES[ENTER]);
  };
